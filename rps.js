...
class HMACGenerator { //HMAC generator
    constructor() {
...
class Game {
    constructor(moves) {
        ...
        this.newRound();  // Start first round
    }

    newRound() {
        this.commove = this.randomcommove();  // Regenerate computer move
        this.hmacGenerator.key = this.hmacGenerator.genKey();  // Generate a new key
        this.hmac = this.hmacGenerator.genHMAC(this.commove);  // Generate new HMAC with the new key
    }
    ...
    play(user_input) {
        ...
        if (user_input === '0') {
            ...
            console.log(result);
            console.log(`HMAC key (for this round): ${this.hmacGenerator.key}`);
